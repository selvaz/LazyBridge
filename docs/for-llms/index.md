...
| **`llms.txt`** | <https://lazybridge.com/llms.txt> | Any tool following the [llmstxt.org](https://llmstxt.org) convention — discovers every doc page with one fetch |
| **`llms-full.txt`** | <https://lazybridge.com/llms-full.txt> | The whole docs corpus concatenated; paste into a long-context model when you don't have a tool that supports `llms.txt` |
