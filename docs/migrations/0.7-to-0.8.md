...
LazyBridge 0.8.0 is the **simplification release**.  The framework
had no real users yet, so we ship breaking changes without
deprecation paths or shims.  Net public surface change: **−2** in
`lazybridge.__all__` (50 → 48), plus 5 deleted `Agent.from_*`
...
# 0.8.0 — Plan kwargs live on Plan, Agent kwargs on Agent
pipeline = Agent(
    engine=Plan(
        Step("research"),
        Step("write"),
...
# 0.8.0 — only the real factory remains
...
# 0.8.0 — flat kwargs (no precedence game, no _UNSET sentinel)
agent = Agent(
    engine=LLMEngine("claude-opus-4-7"),
    timeout=60,
    max_retries=5,
...
# 0.8.0 — both ``tool()`` and ``Tool()`` default to mode="signature"
...
```python
# 0.8.0 — single Envelope (matches the framework invariant)
fan = Agent.parallel(researcher_us, researcher_eu, researcher_asia)
...
# 0.8.0 — wrap_tool is private (_wrap_tool); use tool() factory
...
| 0.7.x behaviour | 0.8.0 behaviour |
|---|---|
...
# 0.8.0 — must pass an explicit name
pipeline = Agent(
...
# 0.8.0 — raises ValueError; pass model on the engine itself
...
The Phase 2 codemods used to migrate the LazyBridge test suite
itself are reusable.  See
[`tools/migrate_07_to_08.py`](https://github.com/selvaz/LazyBridge/blob/main/tools/migrate_07_to_08.py)
(planned for the v0.8.0 tag) for a script that rewrites the most
