...
| `wrap_tool(...)` | private `_wrap_tool` | use `tool(...)` instead |
...
```python
# 0.7.x
search = tool(search_web, name="search")  # default mode="auto"
# silently degrades signature → hybrid → llm depending on schema_llm

# 0.7.9 — both ``tool()`` and ``Tool()`` default to mode="signature"
search = tool(search_web, name="search")
...
# 0.7.9 — wrap_tool is private (_wrap_tool); use tool() factory
from lazybridge import tool
t = tool(some_callable_or_agent, name="...")  # name= required for callables
```
...
- `tool(fn, name=...)` factory unchanged surface; default mode now
...
- `Tool` and `tool` both still public; `tool()` is the canonical
  construction path.
