...
| `ToolTimeoutError` | A tool exceeded `Tool(timeout=N)`, `Agent(tool_timeout=N)` or `LLMEngine(tool_timeout=N)` — the most specific one set wins | Runtime, inside tool dispatch | The engine catches this internally and reports it to the model as `[TOOL_TIMEOUT] …` in the tool result, so the model can recover; the agent run does not abort. Catch only if you wrap the engine yourself |
