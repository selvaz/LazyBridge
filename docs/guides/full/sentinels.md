...
| `from_parallel("n")` | Same as `from_step("n")` — different name signals intent | Plan execution history | as above |
| `from_parallel_all("n")` | Every consecutive parallel sibling starting at `"n"`, joined as labelled text | Plan execution history | `"n"` must be the FIRST step of the band |
