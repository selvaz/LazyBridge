...
from lazybridgeframework.graph.schema import GraphSchema
from lazybridgeframework.lazy_store import LazyStore

if TYPE_CHECKING:
    from lazybridgeframework.lazy_agent import LazyAgent
    from lazybridgeframework.lazy_tool import LazyTool
...
# These events flood the log at BASIC level; only emitted when tracking="verbose"
...
class EventLog:
    ...
    def log(
        ...
        **data: Any,
        ...
        skipped unless level is VERBOSE."""
        if self.level == TrackLevel.OFF:
            return
        if self.level == TrackLevel.BASIC and event_type in _VERBOSE_ONLY:
            return
        row = {
            "timestamp":  _now(),
...
class LazySession:
    ...
    def __init__(
        ...
        *,
        ...
        self.events = EventLog(self.id, db=db, level=tracking)
