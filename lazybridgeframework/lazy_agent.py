...
from lazybridgeframework.lazy_session import Event, LazySession
from lazybridgeframework.lazy_tool import LazyTool, NormalizedToolSet
    ...
    **kwargs:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._log = (
            session.events.agent_log(self.id, self.name)
    ...
    def chat(
        ...
        **kwargs: Any,
        ...
        self._track(Event.MODEL_REQUEST, model=request.model, n_messages=len(msgs))
        if stream:
        ...
        **chat_kwargs: Any,
        ...
        for step in range(max_steps):
            ...
            if resp.content and resp.tool_calls:
                _tc_blocks = [ToolUseContent(id=tc.id, name=tc.name, input=tc.arguments)
                              for tc in resp.tool_calls]
                _asst_content: Any = [TextContent(text=resp.content)] + _tc_blocks
            elif resp.tool_calls:
                _asst_content = [ToolUseContent(id=tc.id, name=tc.name, input=tc.arguments)
                                 for tc in resp.tool_calls]
            else:
                _asst_content = resp.content or ""
            convo.append(Message(role=Role.ASSISTANT, content=_asst_content))
