"""Shared types and data structures for lazybridgeframework."""
...
class StructuredOutputValidationError(Exception):
    ...
    def __init__(self, message: str, *, provider: str | None = None, model: str | None = None, raw: str | None = None, parsed: Any = None):
        super().__init__(message)
        self.provider = provider
        self.model = model
        self.raw = raw
...
class CompletionResponse:
    ...
    validated: bool | None = None    # None if structured output was not requested; True on success; False on parse/validation failure
...
class StreamChunk:
    ...
    validated: bool | None = None        # None if structured output was not requested; True on success; False on parse/validation failure
    grounding_sources: list[GroundingSource] = field(default_factory=list)  # Web search citations (populated on is_final=True)
    web_search_queries: list[str] = field(default_factory=list)  # Queries issued by the grounding tool (populated on is_final=True)
