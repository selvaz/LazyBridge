...
Quick start
-----------
    from lazybridge.external_tools.veo import veo_tool
...
def veo_tool(
    api_key: Annotated[str | None, "Google API key; falls back to GOOGLE_API_KEY env var."] = None,
    ...
    if not key:
        raise ValueError("Missing Google API key.  Pass api_key= or set the GOOGLE_API_KEY environment variable.")
    ...
    def generate_veo_video(
        ...
        duration_seconds: Annotated[
            int,
            "Video length in seconds.  Allowed: 4/6/8 (Veo 3.x) or 5/6/8 (Veo 2.x).  Must be 8 when using last_frame, reference_images, or resolution 1080p/4k.",
        ] = 8,
        aspect_ratio: Annotated[
            AspectRatio, "Aspect ratio: '16:9' (landscape, default) or '9:16' (portrait/mobile)."
        ] = "16:9",
        resolution: Annotated[
            Resolution,
            "Output resolution: '720p' (default), '1080p', or '4k'.  Higher resolutions require duration_seconds=8.",
        ] = "720p",
        generate_audio: Annotated[
            bool,
            "Generate an audio track with the video (default True).  Only supported by non-fast Veo 3.x models.  Setting True with a fast/Veo-2 model raises an error.",
        ] = True,
        negative_prompt: Annotated[
            str | None,
            "What to exclude from the video.  Use descriptive nouns, not negations: 'blur, noise, shaky camera' not 'no blur'.",
        ] = None,
        seed: Annotated[
            int | None,
            "NOT supported by the Gemini API — raises ValueError if set.  On Vertex AI (future) this would be a uint32 that anchors the random state, helping produce visually similar output across runs with the same prompt.  Useful for iterating on a prompt while keeping subject/composition stable.",
        ] = None,
        enhance_prompt: Annotated[bool, "Let Veo auto-expand the prompt for richer results (default True)."] = True,
        first_frame: Annotated[str | None, "Local file path or gs:// URI for the first frame (image-to-video)."] = None,
        last_frame: Annotated[
            str | None,
            "Local file path or gs:// URI for the last frame (frame interpolation).  Requires first_frame and duration_seconds=8.",
        ] = None,
        reference_images: Annotated[
            list[str] | None,
            "List of up to 3 local paths or gs:// URIs used as asset references.  Requires duration_seconds=8.",
        ] = None,
        person_generation: Annotated[
            PersonPolicy,
            "Person content policy: 'allow_all' (default), 'allow_adult', or 'dont_allow'.  Veo 3.x text-to-video only supports 'allow_all'.",
        ] = "allow_all",
    ) -> dict:
        """
        ...
        if needs_8s and dur_str != "8":
            raise ValueError(
                f"duration_seconds must be 8 when using last_frame, reference_images, or resolution '{resolution}'"
            )
        ...
        if seed is not None:
            raise ValueError(
                "seed is not supported by the Gemini API — only on Vertex AI. Remove seed= from your call."
            )
        if last_frame:
