...
class AnalysisDiscoveryResult(BaseModel):
    ...
    best_by_aic: str | None = Field(
        default=None,
        description="run_id with lowest AIC",
    )
    best_by_bic: str | None = Field(
        default=None,
        description="run_id with lowest BIC",
    )
    suggestions: list[str] = Field(default_factory=list)
...
class AnalyzeInput(BaseModel):
    ...
    mode: str = Field(
        default="recommend", description="Analysis goal: describe, forecast, volatility, regime, recommend"
    )
...
class FitModelInput(BaseModel):
    ...
    params: dict[str, Any] | None = Field(
        default=None, description="Model-specific parameters (e.g. {'p': 1, 'q': 1} for GARCH)"
    )
...
class RegisterDatasetInput(BaseModel):
    ...
    frequency: str = Field(
        default="daily", description="Data frequency: daily, weekly, monthly, quarterly, annual, intraday, irregular"
    )
...
class ListUniverseInput(BaseModel):
    ...
    asset_class: str | None = Field(
        default=None,
        description="Filter by asset class: EQUITY, FIXED_INCOME, COMMODITIES, REAL_ESTATE, ALTERNATIVES, MACRO, FX",
    )
    sub_asset_class: str | None = Field(
        default=None, description="Filter by sub-asset class (e.g. 'Developed', 'Government', 'Energy')"
    )
