...
For cross-session persistence, serialise via memory.history::
    ...
    # --- next session ---
    mem = Memory.from_history(json.loads(sess.store.read("history") or "[]"))
"""
...
class Memory:
    ...
    @classmethod
    def from_history(cls, messages: list[dict]) -> Memory:
        """Restore a Memory instance from a previously serialised history list.

        Use this instead of writing to ``_messages`` directly::

            mem = Memory.from_history(json.loads(stored_history))
        """
        instance = cls()
