"""
lazybridge.tools.doc_skills  —  Local documentation skill runtime
=================================================================
...
class SkillManifest:
    ...
    extensions:    list[str]
    version:       str = "3.0.0"
    created_by:    str = "lazybridge.tools.doc_skills"
    ...
    for i, (heading, body) in enumerate(pairs):
        ...
        if body:
            ...
            chunks.append(DocChunk(
                ...
                text=body, tokens=toks, doc_len=len(toks), ordinal=i,
            ))
    return chunks
...
def query_skill(
    ...
    if resolved_mode == "locate":
        ...
        for c in selected[:6]:
            snippet = _trim(re.sub(r'\s+', ' ', c.text), 300)
            result_lines.append(f"  • {snippet}  [{Path(c.path).name}]")
    else:
        result_lines = ["Best evidence:"]
        for c in selected[:5]:
            snippet = _trim(re.sub(r'\s+', ' ', c.text), 400)
            result_lines.append(f"  • {snippet}  [{Path(c.path).name}]")
        if include_quotes:
            result_lines.append("\nFull excerpts:")
            for c in selected[:3]:
