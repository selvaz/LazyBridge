"""DAG-builder planner: ``make_planner`` and the five ``execute_plan`` tools.

Give an LLM a list of sub-agents; get back an :class:`lazybridge.Agent`
whose tools are those sub-agents *plus* five **builder** tools that
compose a :class:`lazybridge.Plan` one step at a time:
...
Reading from a parallel band — three patterns
---------------------------------------------
- One specific branch                — ``task_kind="from_step"`` /
                                        ``task_kind="from_parallel"`` (alias).
- Two branches (one task + one context) — ``task_kind="from_parallel"`` for A
                                          plus ``context_kind="from_parallel"``
                                          + ``context_step`` for B.
- ALL branches as a labelled-text join — ``task_kind="from_parallel_all"``
                                          with ``task_step`` set to the FIRST
                                          member of the parallel band.
"""
