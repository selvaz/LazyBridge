"""
doc_skills_tool.py  —  Local documentation skill runtime for LazyBridge
========================================================================
...
Dependencies: none beyond the standard library (and lazybridge for tool/agent wrappers).
"""
...
try:
    ...
    LazyAgent = LazySession = LazyTool = ToolSchemaMode = None  # type: ignore[assignment]
...
# Extensions where heading-aware chunking is applied
HEADING_EXTENSIONS: frozenset[str] = frozenset({".md", ".mdx", ".rst", ".adoc"})

# BM25 hyperparameters (standard values, safe for technical docs)
BM25_K1: float = 1.5
BM25_B:  float = 0.75
...
class SkillManifest:
    ...
    total_chunks:  int
    avgdl:         float   # average chunk token count — needed at query time
    extensions:    list[str]
    version:       str = "3.0.0"
    created_by:    str = "doc_skills_tool"
...
def _tokenize(text: str) -> list[str]:
    """Alphanumeric + underscore + dot tokens, minimum length 2, no stopwords."""
    toks = re.findall(r"[a-zA-Z0-9_.]{2,}", text.lower())
    return [t for t in toks if t not in STOPWORDS]


def _doc_title(path: Path, text: str) -> str:
    """First non-empty heading line, or a humanised filename."""
...
def _char_chunks(text: str, chunk_size: int, overlap: int) -> list[str]:
    """Split text by character count, snapping to paragraph boundaries."""
...
def _heading_chunks(
    text: str, max_chunk: int, overlap: int
) -> list[tuple[str, str]]:
    """
    Split on markdown headings → (heading_text, full_section_text) pairs.

    Sections larger than max_chunk are sub-split by character count.
    Consecutive sections smaller than max_chunk // 4 are merged with their
    neighbour to avoid tiny orphan chunks.
    """
    ...
    # Sub-split oversized sections; merge tiny ones
...
def _make_chunks(
    path: Path, text: str, chunk_size: int, overlap: int
) -> list[DocChunk]:
    ...
    if path.suffix.lower() in HEADING_EXTENSIONS:
        pairs: list[tuple[str, str]] = _heading_chunks(text, chunk_size, overlap)
    else:
        pairs = [("", chunk) for chunk in _char_chunks(text, chunk_size, overlap)]
...
def _build_idf(chunks: list[DocChunk]) -> dict[str, float]:
    ...
    Stored once at index time in vocab.json.  Rare technical terms (class names,
    parameter names, error codes) receive high weight; common words near zero.
    """
    N  = len(chunks)
    df: Counter[str] = Counter()
    for chunk in chunks:
        for tok in set(chunk.tokens):  # set: count each term once per doc
            df[tok] += 1
    return {
...
def _bm25(
    ...
    for tok in q_tokens:
        ...
        if not tf:
            continue
        idf_w    = idf.get(tok, 0.5)
        tf_norm  = tf * (BM25_K1 + 1) / (
            tf + BM25_K1 * (1 - BM25_B + BM25_B * dl / avgdl)
        )
        score += idf_w * tf_norm

    # Section heading match — moderate boost (heading text is dense signal)
    heading_lower = chunk.heading.lower()
    for tok in q_tokens:
        if tok in heading_lower:
            score += 1.2

    # Exact phrase boost — strong signal for technical queries
...
def _iter_docs(
    roots: Sequence[Path], include_exts: Sequence[str]
) -> Iterable[Path]:
...
def _load_manifest(skill_dir: Path) -> SkillManifest:
    return SkillManifest(**json.loads(
...
# ── Context brief (what the LLM receives) ─────────────────────────────────────
...
def _auto_mode(task: str) -> MODE:
    ...
    if any(x in t for x in (
        "where", "find", "locate", "which file", "which doc",
        "in quale", "dove", "trova",
    )):
        return "locate"
    if any(x in t for x in (
        "extract", "list all", "enumerate", "all examples",
        "estrai", "elenca",
    )):
        return "extract"
    if any(x in t for x in (
        "summarize", "summarise", "overview", "brief",
        "riassumi", "panoramica",
    )):
        return "summarize"
    return "answer"
...
def build_skill(
    ...
    chunk_size:    Annotated[int,  "Maximum characters per chunk (heading sections may be smaller)."] = 1800,
    chunk_overlap: Annotated[int,  "Character overlap between consecutive chunks (char-mode only)."] = 180,
    ...
    The bundle contains:
      ...
      chunks.jsonl   — one JSON object per chunk (heading-aware)
      sources/       — optional copy of the original docs
    ...
    skill_dir = out_root / _slugify(skill_name)
    ...
    description = description or (
        f"Documentation skill built from {len(indexed_files)} files."
    )
    ...
    skill_md = _render_skill_md(
        ...
        file_count=len(indexed_files), total_chunks=len(all_chunks),
        source_dirs=roots,
    )
    ...
    (skill_dir / "vocab.json").write_text(
...
def query_skill(
    ...
    include_quotes: Annotated[bool, "Append full excerpts after evidence bullets (answer/summarize modes)."] = True,
) -> str:
    """
    ...
    The brief contains: skill instructions, retrieved source paths, excerpts,
    and an execution policy that constrains the LLM to the indexed evidence.
    """
    ...
    ranked   = sorted(
        ((c, _bm25(c, q_tokens, idf, manifest.avgdl)) for c in chunks),
    ...
    brief = _build_brief(
        manifest=manifest, skill_md=skill_md,
    ...
    if resolved_mode == "locate":
        result_lines = ["Relevant files:"] + [
            f"  • {p}" for p in dict.fromkeys(c.path for c in selected)
        ]
        ...
        for c in selected[:6]:
            result_lines.append(
                f"  • {_trim(re.sub(r'\\s+', ' ', c.text), 300)}  [{Path(c.path).name}]"
            )
        ...
        for c in selected[:5]:
            result_lines.append(
                f"  • {_trim(re.sub(r'\\s+', ' ', c.text), 400)}  [{Path(c.path).name}]"
            )
        if include_quotes:
            result_lines.append("\nFull excerpts:")
            for c in selected[:3]:
...
def skill_tool(
    skill_dir:   Annotated[str,        "Path to a skill bundle created by build_skill()."],
    name:        Annotated[str | None, "Tool name exposed to the agent. Defaults to the skill name."] = None,
    description: Annotated[str | None, "Tool description. Defaults to the skill description."] = None,
    ...
    """
    if LazyTool is None:
        raise RuntimeError("LazyBridge is not installed.")
    ...
    def _run(
        ...
        return query_skill(
            str(sdir), task, mode=mode, top_k=top_k, include_quotes=include_quotes,
        )
...
def skill_builder_tool(
    ...
    guidance:    Annotated[str,  "Guidance for the calling agent."] = (
        "Use this tool to transform one or more documentation folders "
        "into a queryable local skill."
    ),
    strict:      Annotated[bool, "Strict JSON schema validation."] = False,
):
    """
    Create a LazyTool that builds skill bundles from documentation folders.
    Pass it to an orchestrator so it can build skills on demand.
    """
    if LazyTool is None:
        raise RuntimeError("LazyBridge is not installed.")
    return LazyTool.from_function(
        build_skill,
        name=name, description=description, guidance=guidance, strict=strict,
    )
    ...
    *,
    ...
    Two-step pipeline exposed as a single LazyTool:
      ...
      1. Router   — rewrites the user task into a retrieval-optimised query,
                    preserving all technical names (class names, method names,
                    parameter names, error codes).
      2. Executor — calls the skill tool via loop() and synthesises a grounded
                    answer citing source files.

    The chain is wired via sess.as_tool(mode="chain") so the router's output
    becomes the executor's input automatically.  The executor has the skill tool
    configured at construction time, so the chain calls loop() on it.

    Returns a LazyTool that can be passed to any outer agent.
    """
    if LazyAgent is None or LazySession is None:
        raise RuntimeError("LazyBridge is not installed.")
    ...
    # tools= at construction → chain calls loop() automatically
    executor = LazyAgent(
        provider,
        name="skill_executor",
        ...
        tools=[s_tool],
        system=(
            "You answer from the local skill tool only. "
            "Always call the skill tool first. "
    ...
    return sess.as_tool(
        ...
        guidance=(
            "Use for questions that should be grounded in the indexed documentation. "
            "The pipeline sharpens the query first, then retrieves and synthesises the answer."
        ),
    )


# ── Usage examples (not executed on import) ────────────────────────────────────

_EXAMPLE_BUILD = """
from doc_skills_tool import build_skill

meta = build_skill(
    source_dirs=["./docs", "./reference"],
    skill_name="my-project",
    description="Local documentation skill for MyProject.",
    usage_notes="Prefer concrete API names and parameter names over prose.",
)
print(meta["skill_dir"], meta["total_chunks"], "chunks", "avgdl:", meta["avgdl"])
"""

_EXAMPLE_TOOL = """
from doc_skills_tool import skill_tool
from lazybridge import LazyAgent

tool = skill_tool("./generated_skills/my-project")
resp = LazyAgent("anthropic").loop(
    "How does session.as_tool(mode='chain') wire context between agents?",
    tools=[tool],
)
print(resp.content)
"""
...
_EXAMPLE_PIPELINE = """
from doc_skills_tool import skill_pipeline
from lazybridge import LazyAgent
...
resp = LazyAgent("anthropic").loop(
    "Find the canonical pattern for a sequential multi-agent pipeline.",
    tools=[pipeline],
)
print(resp.content)
"""
