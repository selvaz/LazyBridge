...
| Phase 3 — validation parity + provider consistency + observability | **in progress** | 2026-05-10 | | Block H done (commit `57e328b` — T5 parallel-band sibling check, T6 model+non-LLM, T7 name=required, I6 existence-not-truthiness, standard error format).  Block I done (capability ClassVars, T9 WEB_SEARCH/GOOGLE_SEARCH dedup warning, `_compute_cost(cached_input_tokens=)` parity on Google/LMStudio/base, I5 Anthropic adaptive-thinking warning corrected, new `lazybridge.matrix` module + `tests/unit/test_public_api_snapshot.py`).  Block J pending. |
