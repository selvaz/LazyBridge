...
from lazybridge.external_tools.report_builder.citations import (
    _crossref_to_csl,
    _extract_doi,
    _openalex_to_csl,
    _safe_key,
...
class TestEnrichFromUrl:
    def test_falls_back_to_minimal_when_lookups_fail(self):
        ...
        with (
            patch("lazybridge.external_tools.report_builder.citations._crossref_lookup", return_value=None),
            patch("lazybridge.external_tools.report_builder.citations._openalex_lookup", return_value=None),
        ):
            store = Store(db=None)
    ...
    def test_caches_result(self):
        ...
        with patch(
            "lazybridge.external_tools.report_builder.citations._crossref_lookup",
            return_value={
                "title": ["Cached"],
                "issued": {"date-parts": [[2024]]},
                "author": [],
        ...
        with (
            ...
            patch(
                "lazybridge.external_tools.report_builder.citations._openalex_lookup",
                return_value=None,
            ),
        ):
